...
def login():
    ...
    query_result = db.session.query(main_user).filter(
        main_user.UserName == auth.username, main_user.Password == auth.password).first()

    if not query_result:
        return make_response('Verification failed!', 401, {'www-Authenticate': 'Basic realm = "Login Required"'})

    token = jwt.encode({'UserName': query_result.UserName, 'exp': datetime.datetime.utcnow(
...
def register():
    ...
    if request.method == 'POST':
        ...
        username_query_result = db.session.query(main_user).filter(
            main_user.UserName == data['UserName']).first()
        if username_query_result:
            return jsonify({'message': 'Username you entered exist in database. Please enter another!'}), 400

        email_query_result = db.session.query(main_user).filter(
            main_user.EmailAddress == data['EmailAddress']).first()
